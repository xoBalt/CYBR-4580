...
class TCPPacket:
 ...
 def assemble_tcp_feilds(self):
  self.raw = struct.pack('!HHLLBBHHHB', # Data Structure Representation
   ...
   self.tcp_options # TCP Urgent Pointer
   )
 ...
 def reassemble_tcp_feilds(self):
  ...
  struct.pack('!H', 
   self.tcp_urg_ptr) +
  struct.pack(self.tcp_options("B"))
  return
 ...
 def create_tcp_feilds(self):
  ...
  self.tcp_urg_ptr = 0
  # ---- [ TCP Urgent Pointer ]
  tcp_options_Experiment = 253
  tcp_options_Experiment_length = 0 #where the numbering of the TCP packets should go
   
  
  self.tcp_optionskind = tcp_options_Experiment + tcp_options_Experiment_length
